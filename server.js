...
client
  ...
  .then(() => {
    client.fetchApplication().then(init);
  })
  .catch(handleError);
      ...
      try {
        ...
        function flatten(pattern, context) {
          Object.keys(context).forEach(key =>
            grammar.pushRules(key, context[key])
          );
        ...
        const requestListener = function(req, res) {
          fs.readFile("index.html", { encoding: "utf-8" }, (err, html) => {
            if (err) {
              ...
              var outputs = Object.keys(mergedOrigins).map(merge => {
                return {
        ...
        function dm(message) {
          ...
          for (var i = 0; i < keys.length; i++) {
            ...
            if (message.content.toLowerCase().includes(origin.toLowerCase())) {
              message.channel.send(
                flatten(settings.origins[origin], {
          ...
          if (settings.origins.hasOwnProperty("default")) {
            message.channel.send(
              flatten(settings.origins.default, {
                user: message.author.username,
                userid: `<@${message.author.id}>`,
                botname: client.user.username,
                botid: client.user.id,
        ...
        client.on("message", message => {
          ...
          var unmentioned =
            message.mentions.members.array().length == 0 &&
            Math.random() < settings.unmentionedProbability;
          if (
            unmentioned ||
            ...
            for (var i = 0; i < keys.length; i++) {
              var origin = keys[i];
              if (
                message.content.toLowerCase().includes(origin.toLowerCase())
              ) {
                message.channel.send(
                  flatten(settings.origins[origin], {
                    user: message.author.username,
